(async () => {
  ...
  async function save(remove){
    if(saving)return;if(!remove&&!text.value.trim()){text.reportValidity();return;}
  ...
  function insertLink(){
    ...
    if(!/^(https?:\/\/|mailto:|\/(?!\/)|#)/i.test(url)||/[\s<>]/.test(url)){panel.querySelector('#link-error').textContent='Enter an http, https, email, or relative URL.';return;}
    ...
    fields.hidden=true;dirty=true;text.focus();renderPreview();
  }
